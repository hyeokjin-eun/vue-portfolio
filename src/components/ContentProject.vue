<template>
    <div class=" text-center title-font" id="project">
        GitHub Project
        <div style="margin: 20px 0px 20px 0px;">
        <div class="align-items-center container">
            <div class="row" style="padding: 0px 10px 0px 10px;">
                <div class="col-sm-4" style="display: flex;">
                    <div class="project-content">
                        <div class="project-content-title">
                            <a href="https://github.com/hyeokjin-eun/eatgo-project-fc">
                                eatgo-project-fc
                            </a>
                        </div>
                        <hr>
                        <div class="project-content-description">
                            Java 학습을 위한 프로젝트로 BDD 개발 방법론과 MSA, JPA,
                            JWT, Spring Boot, Spring Security 등을 활용한 가게 예약 프로젝트 입니다.
                        </div>
                    </div>
                </div>
                <div class="col-sm-4" style="display: flex;">
                    <div class="project-content">
                        <div class="project-content-title">
                            <a href="https://github.com/hyeokjin-eun/admin-project-fc">
                                admin-project-fc
                            </a>
                        </div>
                        <hr>
                        <div class="project-content-description">
                            Java 학습을 위한 프로젝트로 BDD 개발 방법론과 JPA, Spring Boot, Swagger 등을
                            활용한 어드민 프로젝트 입니다.
                        </div>
                    </div>
                </div>
                <div class="col-sm-4" style="display: flex;">
                    <div class="project-content">
                        <div class="project-content-title">
                            <a href="https://github.com/hyeokjin-eun/java-board">
                                java-board
                            </a>
                        </div>
                        <hr>
                        <div class="project-content-description">
                            Java Toy 프로젝트로 BDD 개발 방법론과 JWT, 
                            JPA, Spring Boot, Spring Security, Spring Hateoas, DDD, Swagger, MSA 등을
                            활용하여 Restful API 개발을 위한 Java 프로젝트 입니다.
                        </div>
                    </div>
                </div>
            </div>
            <div class="row" style="padding: 0px 10px 0px 10px;">
                <div class="col-sm-4" style="display: flex;">
                    <div class="project-content">
                        <div class="project-content-title">
                            <a href="https://github.com/hyeokjin-eun/python-base-fc">
                                python-base-fc
                            </a>
                        </div>
                        <hr>
                        <div class="project-content-description">
                            Python 학습을 위한 프로젝트로 기초 문법 학습을 위한 프로젝트 입니다.
                        </div>
                    </div>
                </div>
                <div class="col-sm-4" style="display: flex;">
                    <div class="project-content">
                        <div class="project-content-title">
                            <a href="https://github.com/hyeokjin-eun/kotlin-board">
                                kotlin-board
                            </a>
                        </div>
                        <hr>
                        <div class="project-content-description">
                            Kotlin Toy 프로젝트로 BDD 개발 방법론과 JWT, JPA, Spring Boot,
                            Swagger를 활용하여 Web API 개발을 위한 Kotlin 프로젝트 입니다.
                        </div>
                    </div>
                </div>
                <div class="col-sm-4" style="display: flex;">
                    <div class="project-content">
                        <div class="project-content-title">
                            <a href="https://github.com/hyeokjin-eun/vue-portfolio">
                                vue-portfolio
                            </a>
                        </div>
                        <hr>
                        <div class="project-content-description">
                            Vue 포트폴리오 프로젝트로 현재 보고 있는 포트폴리오 프로젝트 입니다.
                        </div>
                    </div>
                </div>
            </div>
            <div class="row" style="padding: 0px 10px 0px 10px;">
                <div class="col-sm-4" style="display: flex;">
                    <div class="project-content">
                        <div class="project-content-title">
                            <a href="https://github.com/hyeokjin-eun/vue-start-study">
                                vue-start-study
                            </a>
                        </div>
                        <hr>
                        <div class="project-content-description">
                            Vue 학습을 위한 프로젝트로 Vue의 기본적인 Component, Props, Event,
                            Computed, Watch, Vue-CLI 등을 학습하기 위한 프로젝트 입니다.
                        </div>
                    </div>
                </div>
            </div>
        </div>
        </div>
    </div>
</template>

<script>
export default {

}
</script>

<style>
    .project-content {
        margin: 10px;
        padding: 15px 10px 10px 10px;
        border: 1px solid lightgray;
        border-radius: 15px;
    }
    .project-content-title {
        font-size: 20px;
    }
    .project-content-description {
        font-size: 17px;
    }
</style>